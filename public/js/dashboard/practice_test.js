...
$(function() {
	...
	var imagepath=base_url+'/public/images/';
  	$('#practice_test_listing').on( 'processing.dt', function ( e, settings, processing ) {
	        ...
	        "deferRender": true,
	        "ajax": {
	            "url": base_url+"/admin/practiceTest",
	            "type": "POST",
	            "dataType": 'json',
	            "data": function ( d ) {
	            ...
	            { "data": "action", sortable:!1,
	              render: function (data, type, row) {
	                ...
	                if (row.edit_access==true) {
	                  	editExam = '<a href="'+base_url+'/admin/editPracticeTest/'+row.exam_id+'"><img src="'+imagepath+'/ic_mode_edit.png"></a>'
	                }
					if (row.student_access) {
						student = '<a href="'+base_url+'/admin/practiceTest/appearedStudents/'+row.exam_id+'"><img src="'+imagepath+'/ic_users_color.png"></a>'
					}
	                viewExam = '<a href="'+base_url+'/admin/viewPracticeTest/'+row.exam_id+'"><img src="'+imagepath+'/ic_eye_color.png"></a>'
	...
	//LISTING
  	$('#appear_stu_listing').on( 'processing.dt', function ( e, settings, processing ) {
	        ...
	        "deferRender": true,
	        "ajax": {
	            "url": base_url+"/admin/practiceTest/appearedStudents",
	            "type": "POST",
	            "dataType": 'json',
	            "data": function ( d ) {
	        ...
	        columns:[
	        	...
	        	{ "data": "action",sortable:!1,className: "text-center",
	            	render: function (data, type, row) {
	                  return'<a href="'+base_url+'/admin/practice/viewUserTest/'+row.user_attempt_id+'" class"text-center"><img src="'+imagepath+'/ic_eye_color.png"></a>'
	                }
	        	},
	...
	//ADD TEST
	$("form[name='add-practice-test-form']").validate({
	  errorClass: "error_msg",
	   rules: {
	    ...
	    $.ajax({
	        url: base_url+'/admin/addPracticeTestPost',
	        type: 'POST',
	        processData: false,
	        contentType: false,
	        cache: false,              
	        ...
	        success: function(result)
	        {
	            if(result.status){
	              ...
	              $('li a[data-slug="admin/practiceTest"]').trigger("click");
	            }else{
...
$(document).on('change','.getCategory', function() {
  ...
  showLoader(true);
  $.ajax({
    url: base_url+'/admin/practiceTest/getCategoryData',
    type: 'POST',
    dataType:'json',
    cache: false,              
    data: {'cid':cid},
...
function confirmDelete(){
  ...
  var cid = $('#did').val();
  $.ajax({
    url: base_url+'/admin/deletePracticeTest',
    type: 'POST',
    dataType:'json',
    cache: false,              
    data: {'cid':cid},
...
function confirmStatus(cid){
  ...
  var cid = $('#did').val();
  $.ajax({
    url: base_url+'/admin/statusPracticeTest',
    type: 'POST',
    dataType:'json',
    cache: false,              
    data: {'cid':cid},
