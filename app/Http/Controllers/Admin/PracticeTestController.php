...
{
	/**
     * Used for Admin Practice test
     * @return redirect to Admin->practiceTest
     */
    ...
    {   
        ...
        if (!empty($accessPriData) && Auth::user()->user_type==1) {
            $cateIdForQue = $accessPriData['Practice_Test']->categories==true ? $accessPriData['Practice_Test']->categories : [];
        }
        $getCategoryWithSubCat = FrontHelper::getCategoryWithSubCat($category,$cateIdForQue);

        if (request()->ajax()) {
            return \View::make('admin.practiceTest.index')->with(['getCategoryWithSubCat'=>$getCategoryWithSubCat])->renderSections();
        }
        return view('admin.practiceTest.index')->with(['getCategoryWithSubCat'=>$getCategoryWithSubCat]);
    }

    /**
     * Used for Admin addPracticeTest
     * @return redirect to Admin->addPracticeTest
     */
    public function addPracticeTest(Request $request)
    {
        ...
        {
            $getCatgory = $accessPriData['Practice_Test']->categories==true ? $accessPriData['Practice_Test']->categories: [] ;
        }
        ...
        if (request()->ajax()) {
            return \View::make('admin.practiceTest.addPracticeTest')->with(['parent_category'=>$parent_category])->renderSections();
        }
        return view('admin.practiceTest.addPracticeTest')->with(['parent_category'=>$parent_category]);
    }
    ...
    public function getCategoryData(Request $request)
    {
        ...
        foreach ($subCate as $key => $value) {
            ...
            $cnt_categoryWiseQue = Question::where('category_id',$value->category_id)->where('question_type','PRACTICE_TEST')->count();
            $cnt_categoryWiseUnusedQue = Question::where('category_id',$value->category_id)->where('question_type','PRACTICE_TEST')->where('isAssignable',true)->where('status',1)->count();
    ...
    /**
     * Used for Admin add addPracticeTestPost
     * @return redirect to Admin->add addPracticeTestPost
     */
    public function addPracticeTestPost(Request $request)
    {
        $response = [];
        $input = $request->all();
        //dd($input);
        ...
        try
        {
            ...
            if(!empty($input['pkCat']) && $input['pkCat'] != null)
            {
                ...
                $addExam['exams_type'] = 'PRACTICE_TEST';
                ...
                foreach ($input['sub_category_id'] as $key=> $value) {
                    $limit = 0;
                    $getQueCount = Question::where('exam_id',$input['pkCat'])->where('question_type','PRACTICE_TEST')->where('category_id',$value)->where('isAssignable',false)->count();

                    if ($getQueCount < $input['question_number'][$key]) {
                        $limit = $input['question_number'][$key]-$getQueCount;
                        $getAllQuetion = Question::inRandomOrder()->where('question_type','PRACTICE_TEST')->where('category_id',$value)->where('isAssignable',true)->where('status',1)->limit($limit)->get();
                    ...
                    }elseif($getQueCount > $input['question_number'][$key]){
                        $limit = $getQueCount-$input['question_number'][$key];
                        Question::where('exam_id',$input['pkCat'])->where('question_type','PRACTICE_TEST')->where('category_id',$value)->where('isAssignable',false)->limit($limit)->update(['isAssignable'=>true]);
                    }
                }
                
                //UPDATE ALL QUESTION WITH EXAM ID TO EXAM QUESTION TABLE.
                $exam_questions = [];
                $getAllQueOfExam = Question::select('questions_id','exam_id')->where('question_type','PRACTICE_TEST')->where('exam_id',$input['pkCat'])->where('isAssignable',false)->get()->toArray();
                ...
                $total_marks = DB::table('questions')->where('isAssignable',false)->where('exam_id',$input['pkCat'])->where('question_type','PRACTICE_TEST')->sum('marks');
            ...
            else
            {
                ...
                $addExam->exams_type = 'PRACTICE_TEST';
                $addExam->total_questions = array_sum($input['question_number']);
                $addExam->total_marks = 0;
                
                if ($addExam->save()) {
                    //UPDATE SELECTED QUESTION
                    foreach ($input['sub_category_id'] as $key=> $value) {
                        $limit = $input['question_number'][$key];
                        $getAllQuetion = Question::inRandomOrder()->where('question_type','PRACTICE_TEST')->where('category_id',$value)->where('isAssignable',true)->where('status',1)->limit($limit)->get();
                    ...
                    $exam_questions = [];
                    $getAllQueOfExam = Question::select('questions_id','exam_id')->where('isAssignable',false)->where('question_type','PRACTICE_TEST')->where('exam_id',$addExam->exam_id)->get()->toArray();
                    foreach ($getAllQueOfExam as $key => $value) {
                        $exam_questions[$key]['questions_id'] = $value['questions_id'];
                        $exam_questions[$key]['exam_id'] = $value['exam_id'];
                        $exam_questions[$key]['created_at'] = now();
                    ...
                    $total_marks = DB::table('questions')->where('exam_id',$addExam->exam_id)->where('isAssignable',false)->where('question_type','PRACTICE_TEST')->sum('marks');
                    ...
                    if ($exam_desc) {
                        ...
                        $cat_id = isset($input['sub_category_id'][0]) ? $input['sub_category_id'][0] : false;
                        $getUserCate = User::with(array('user_category'=>function($queCat) use($getAllCateId,$cat_id){
                            $queCat->where('category_id',$cat_id);
                        ...
                        $data = EmailMaster::where('email_key', 'add_new_practice_test')->first();
                        if (isset($data) && !empty($data) && !empty($getUserCate) && count($getUserCate)>0) {
                            ...
                            $aNotificaTransMsg = Translation::where('group','notification_message')->where('key','practice_test_add_msg')->first()->toArray();
                            $aNotificaTransTitle = Translation::where('group','notification_title')->where('key','practice_test_add_title')->first()->toArray();
                            foreach ($getUserCate as $key => $value) {
                                $uName = $value->name ?? 'NA';
    ...
    */
    public function getPracticeTest(Request $request)
    {
       ...
       */
        ...
        $aTable = Exam::with('desc','category_desc')->where('exams_type','PRACTICE_TEST');
        ...
        if($accessPriData){
            ...
            if(!empty($accessPriData['Practice_Test']) && $accessPriData['Practice_Test']->view==true)
            {
                $getAcc['STATUS'] = $accessPriData['Practice_Test']->status==true ? true:false;
                $getAcc['DELETED'] = $accessPriData['Practice_Test']->delete==true ? true:false;
                $getAcc['EDIT'] = $accessPriData['Practice_Test']->edit==true ? true:false;
                $getAcc['STUDENT'] = $accessPriData['Practice_Test']->students==true ? true:false; 

                $cateIdForLiveQue = $accessPriData['Practice_Test']->categories==true ? $accessPriData['Practice_Test']->categories : [];
        ...
        if ($test_type) {
            $getAllTestId = Exam::select('exam_id')->where('exams_type','PRACTICE_TEST')->get()->toArray();
        ...
        foreach ($aTables as $key => $value) {
            ...
            $value['category'] = $categoryIdForDoubt;
    ...
    /**
     * Used for Admin viewPracticeTest
     * @return redirect to Admin->edit viewPracticeTest
     */
    public function viewPracticeTest(Request $request,$id)
    {
        $testData = [];
        $no_of = [];
        $getQueCate = [];
        ...
        return view('admin.practiceTest.viewPracticeTest')->with(['cate'=>$categoryData,'getQueCate'=>$getQueCate,'getExam'=>$getExam,'testData'=>$testData,'no_of'=>$no_of]);
    
    }


    /**
     * Used for Admin editPracticeTest
     * @return redirect to Admin->edit editPracticeTest
     */
    public function editPracticeTest(Request $request, $id)
    {
        ...
        $getCateWiseQueData = Category::select('category_id')->where('category_id',$getExam['category_id'])->get()->toArray();
        foreach ($getCateWiseQueData as $key => $value) {
            ...
            $no_of[$key]['total_quetion'] = Question::where('category_id',$value['category_id'])->where('question_type','PRACTICE_TEST')->count();        

            $no_of[$key]['unused_que'] = Question::where('category_id',$value['category_id'])->where('question_type','PRACTICE_TEST')->where('status',1)->where(function ($query) use($id){
                        $query->where('isAssignable',true)->orwhere('exam_id',$id)
                        ->orWhereNull('exam_id');
                    })->count();            

            $no_of[$key]['used_que'] = Question::where('category_id',$value['category_id'])->where('question_type','PRACTICE_TEST')->where('exam_id',$getExam['exam_id'])->where('isAssignable',false)->count();
        }

        return view('admin.practiceTest.editPracticeTest')->with(['getSubPare'=>$getSubPare,'parent_category'=>$parent_category,'getExam'=>$getExam,'no_of'=>$no_of]);
    }



    /**
     * Used for Admin deletePracticeTest
     * @return redirect to Admin->delete deletePracticeTest
     */
    public function deletePracticeTest(Request $request)
    {
        $input = $request->all();
        $cid = $input['cid'];
        $response = [];
        ...
        if(empty($cid)){
            ...
            if (empty($userAttempt) && $userAttempt <= 0) {
                ...
                $updated = Question::where('exam_id',$cid)->where('question_type','PRACTICE_TEST')->update(['isAssignable'=>true]);
                if($updated){
                    $response['status'] = true;
                    $response['message'] = "Practice test Successfully deleted";
                }else{
                    $response['status'] = false;
                    $response['message'] = "Something Went Wrong";
                }
    ...
    */
    public function statusPracticeTest(Request $request)
    {
        $input = $request->all();
        $cid = $input['cid'];
        $response = [];
    ...
    */
    public function appearedStudents(Request $request, $id)
    {
        ...
        if (request()->ajax()) {
            return \View::make('admin.practiceTest.appearedStuList')->with(['appearedStu'=>$appearedStu])->renderSections();
        }
        return view('admin.practiceTest.appearedStuList')->with(['appearedStu'=>$appearedStu]);
    ...
    */
    ...
    public function viewUserTest(Request $request,$id)
    {
        ...
        if (request()->ajax()) {
            return \View::make('admin.practiceTest.viewUserTest')->with(['getAttemptedOption'=>$getAttemptedOption,'data'=>$data,'cate'=>$categoryData,'getQueCate'=>$getQueCate,'getExam'=>$getExam,'testData'=>$testData,'no_of'=>$no_of])->renderSections();
        }
        return view('admin.practiceTest.viewUserTest')->with(['getAttemptedOption'=>$getAttemptedOption,'data'=>$data,'cate'=>$categoryData,'getQueCate'=>$getQueCate,'getExam'=>$getExam,'testData'=>$testData,'no_of'=>$no_of]);
    }
