...
class QuizzController extends Controller
{
    ...
    {	
        ...
        if (!empty($accessPriData) && Auth::user()->user_type==1) {
            $cateIdForQue = $accessPriData['Quizz_Test']->categories==true ? $accessPriData['Quizz_Test']->categories : [];
        }
        $getCategoryWithSubCat = FrontHelper::getCategoryWithSubCat($category,$cateIdForQue);

        if (request()->ajax()) {
            return \View::make('admin.quizTest.index')->with(['getCategoryWithSubCat'=>$getCategoryWithSubCat])->renderSections();
        }
    	return view('admin.quizTest.index')->with(['getCategoryWithSubCat'=>$getCategoryWithSubCat]);
    }
    ...
    public function addQuizTest(Request $request)
    {
        ...
        {
            $getCatgory = $accessPriData['Quizz_Test']->categories==true ? $accessPriData['Quizz_Test']->categories : [];
        }
    	...
    	if (request()->ajax()) {
            return \View::make('admin.quizTest.addQuizTest')->with(['parent_category'=>$parent_category])->renderSections();
        }
    	return view('admin.quizTest.addQuizTest')->with(['parent_category'=>$parent_category]);
    }
    ...
    public function getCategoryData(Request $request)
    {
        ...
        foreach ($subCate as $key => $value) {
            ...
            $cnt_categoryWiseQue = Question::where('category_id',$value->category_id)->where('question_type','QUIZZES')->count();
            $cnt_categoryWiseUnusedQue = Question::where('category_id',$value->category_id)->where('question_type','QUIZZES')->where('isAssignable',true)->where('status',1)->count();
    ...
    public function addQuizTestPost(Request $request)
    {
        ...
        try{
            ...
            if(!empty($input['pkCat']) && $input['pkCat'] != null)
            {
                ...
                $addExam['exams_type'] = 'QUIZZES';
                $addExam['exam_duration'] = $input['duration'];
                $addExam['total_questions'] = array_sum($input['question_number']);
                
                //UPDATE SELECTED QUESTION
                //Question::where('exam_id',$input['pkCat'])->update(['exam_id'=>null,'isAssignable'=>true]);
                foreach ($input['sub_category_id'] as $key=> $value) {
                    $limit = 0;
                    $getQueCount = Question::where('exam_id',$input['pkCat'])->where('question_type','QUIZZES')->where('category_id',$value)->where('isAssignable',false)->count();

                    if ($getQueCount < $input['question_number'][$key]) {
                        $limit = $input['question_number'][$key]-$getQueCount;
                        $getAllQuetion = Question::inRandomOrder()->where('question_type','QUIZZES')->where('category_id',$value)->where('isAssignable',true)->where('status',1)->limit($limit)->get();
                        ...
                        Question::where('exam_id',$input['pkCat'])->where('question_type','QUIZZES')->where('category_id',$value)->where('isAssignable',false)->limit($limit)->update(['isAssignable'=>true]);
                    }
                }

                //UPDATE ALL QUESTION WITH EXAM ID TO EXAM QUESTION TABLE.
                $exam_questions = [];
                $getAllQueOfExam = Question::select('questions_id','exam_id')->where('exam_id',$input['pkCat'])->where('question_type','QUIZZES')->where('isAssignable',false)->get()->toArray();
                ...
                $total_marks = DB::table('questions')->where('question_type','QUIZZES')->where('isAssignable',false)->where('exam_id',$input['pkCat'])->sum('marks');
            ...
            else
            {
                ...
                if ($addExam->save()) {
                    ...
                    $exam_questions = [];
                    $getAllQueOfExam = Question::select('questions_id','exam_id')->where('question_type','QUIZZES')->where('isAssignable',false)->where('exam_id',$addExam->exam_id)->get()->toArray();
                    foreach ($getAllQueOfExam as $key => $value) {
                        $exam_questions[$key]['questions_id'] = $value['questions_id'];
                        $exam_questions[$key]['exam_id'] = $value['exam_id'];
                        $exam_questions[$key]['created_at'] = now();
                    ...
                    $total_marks = DB::table('questions')->where('exam_id',$addExam->exam_id)->where('question_type','QUIZZES')->where('isAssignable',false)->sum('marks');
                    ...
                    if ($exam_desc) {
                        ...
                        $data = EmailMaster::where('email_key', 'add_new_quiz_test')->first();
                        if (isset($data) && !empty($data) && !empty($getUserCate) && count($getUserCate)>0) {
                            ...
                            $aNotificaTransMsg = Translation::where('group','notification_message')->where('key','quiz_test_add_msg')->first()->toArray();
                            $aNotificaTransTitle = Translation::where('group','notification_title')->where('key','quiz_test_add_title')->first()->toArray();
                            ...
                            foreach ($getUserCate as $key => $value) {
                                ...
                                if (!empty($value->device_token) || $value->device_token==!null) {
                                    ...
                                    /*==GET EXAM CATEGORY NAME ACCRODING TO USER LANG==*/
                                    $getExamCategoryName = CategoriesDesc::select('name')->where('category_id',$addExam->category_id)->where('lang_code',$userLangCode)->first();
                                    /* --END */
                                    ...
                                    $notification_data = [
                                        'action' => 'QUIZ_NEW_EXAM',
                                        'exam_id' =>  $addExam->exam_id,
                                        'level_id' => $addExam->category_id,
                                        'level_name' => $getExamCategoryName->name
                                    ];
    ...
    */
    public function getQuizTest(Request $request)
    {
       ...
       */
        ...
        $aTable = Exam::with('desc','category_desc')->where('exams_type','QUIZZES');
        ...
        if($accessPriData){
            ...
            if(!empty($accessPriData['Quizz_Test']) && $accessPriData['Quizz_Test']->view==true)
            {
                $getAcc['STATUS'] = $accessPriData['Quizz_Test']->status==true ? true:false;
                $getAcc['DELETED'] = $accessPriData['Quizz_Test']->delete==true ? true:false;
                $getAcc['EDIT'] = $accessPriData['Quizz_Test']->edit==true ? true:false;
                $getAcc['STUDENT'] = $accessPriData['Quizz_Test']->students==true ? true:false; 


                $cateIdForLiveQue = $accessPriData['Quizz_Test']->categories==true ? $accessPriData['Quizz_Test']->categories : [];
        ...
        if ($test_type) {
            $getAllTestId = Exam::select('exam_id')->where('exams_type','QUIZZES')->get()->toArray();
    ...
    public function viewQuizTest(Request $request,$id)
    {
        ...
        return view('admin.quizTest.viewQuizTest')->with(['cate'=>$categoryData,'getQueCate'=>$getQueCate,'getExam'=>$getExam,'testData'=>$testData,'no_of'=>$no_of]);
    ...
    public function editQuizTest(Request $request, $id)
    {
        ...
        foreach ($getCateWiseQueData as $key => $value) {
            ...
            $no_of[$key]['total_quetion'] = Question::where('category_id',$value['category_id'])->where('question_type','QUIZZES')->count();        

            $no_of[$key]['unused_que'] = Question::where('category_id',$value['category_id'])->where('question_type','QUIZZES')->where('status',1)->where(function ($query) use($id){
                        $query->where('isAssignable',true)->orwhere('exam_id',$id)
                        ->orWhereNull('exam_id');
                    })->count();

            $no_of[$key]['used_que'] = Question::where('category_id',$value['category_id'])->where('question_type','QUIZZES')->where('exam_id',$getExam['exam_id'])->where('isAssignable',false)->count();
        }

        return view('admin.quizTest.editQuizTest')->with(['getSubPare'=>$getSubPare,'parent_category'=>$parent_category,'getExam'=>$getExam,'no_of'=>$no_of]);
    }
    ...
    public function deleteQuizTest(Request $request)
    {
        ...
        if(empty($cid)){
            ...
            if (empty($userAttempt) && $userAttempt <= 0) {
                ...
                $updated = Question::where('exam_id',$cid)->where('question_type','QUIZZES')->update(['isAssignable'=>true]);
                if($updated){
                    $response['status'] = true;
    ...
    */
    public function appearedStudents(Request $request, $id)
    {
        ...
        return view('admin.quizTest.appearedStuList')->with(['appearedStu'=>$appearedStu]);
    ...
    */
    ...
    public function viewUserTest(Request $request,$id)
    {
        ...
        if (request()->ajax()) {
            return \View::make('admin.quizTest.viewUserTest')->with(['getAttemptedOption'=>$getAttemptedOption,'data'=>$data,'cate'=>$categoryData,'getQueCate'=>$getQueCate,'getExam'=>$getExam,'testData'=>$testData,'no_of'=>$no_of])->renderSections();
        }
        return view('admin.quizTest.viewUserTest')->with(['getAttemptedOption'=>$getAttemptedOption,'data'=>$data,'cate'=>$categoryData,'getQueCate'=>$getQueCate,'getExam'=>$getExam,'testData'=>$testData,'no_of'=>$no_of]);
    }
